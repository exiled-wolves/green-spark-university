...
transporter.verify((error, success) => {
  if (error) {
    console.error('[v0] Email transporter error:', error.message);
  } else {
    console.log('[v0] Email transporter ready to send emails');
  }
});
...
const sendAdmissionEmail = async ({ to, fullName, loginId, password, department }) => {
  try {
    console.log('[v0] Sending admission email to:', to);
        ...
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#F8F9FA;padding:32px;border-radius:8px;">
          ...
          <div style="background:#ffffff;padding:32px;border-radius:0 0 6px 6px;border:1px solid #dee2e6;">
            ...
            <p style="color:#1C1C1C;">
              ...
              <a href="https://green-spark-university-eight.vercel.app" style="color:#1F4E79;">Green Spark University Portal</a>
    ...
    console.error('[v0] Error sending admission email:', error.message);
    throw error;
  }
};
...
const sendRejectionEmail = async ({ to, fullName, department }) => {
  try {
    console.log('[v0] Sending rejection email to:', to);
    ...
    const result = await transporter.sendMail(mailOptions);
    console.log('[v0] Rejection email sent successfully:', result.messageId);
    return result;
  } catch (error) {
    console.error('[v0] Error sending rejection email:', error.message);
    throw error;
  }
};
